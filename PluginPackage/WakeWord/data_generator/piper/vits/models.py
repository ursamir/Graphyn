...
class StochasticDurationPredictor(nn.Module):
    ...
    def forward(
        ...
        if not reverse:
            flows: nn.ModuleList | list[nn.Module] = self.flows
            assert w is not None

            logdet_tot_q: torch.Tensor = torch.tensor(0.0)
            h_w = self.post_pre(w)
            ...
            logdet_tot: torch.Tensor = torch.tensor(0.0)
...
class Generator(torch.nn.Module):
    ...
    def remove_weight_norm(self) -> None:
        ...
        for l in self.resblocks:
            assert isinstance(l, (modules.ResBlock1, modules.ResBlock2))
...
class MultiPeriodDiscriminator(torch.nn.Module):
    def __init__(self, use_spectral_norm: bool = False):
        ...
        discs: list[nn.Module] = [DiscriminatorS(use_spectral_norm=use_spectral_norm)]
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(
        ...
        self.dp: StochasticDurationPredictor | DurationPredictor
        if use_sdp:
    ...
    def infer(
        ...
        y_mask = torch.unsqueeze(commons.sequence_mask(y_lengths, y_lengths.max()), 1).type_as(
            x_mask
        )
        attn_mask = torch.unsqueeze(x_mask, 2) * torch.unsqueeze(y_mask, -1)
