"""McuMfccNode — MCU-friendly MFCC feature extract
...
try:
    ...
    try:
        _types = importlib.import_module("mcu_mfcc.types")
    except (ImportError, ModuleNotFoundError):
...
class McuMfccNode(Node):
    """MCU-friendly MFCC feature extract"""

    node_type: ClassVar[str] = "mcu_mfcc"

    metadata: ClassVar[NodeMetadata] = NodeMetadata(
        node_type="mcu_mfcc",
        label="Mcu Mfcc",
        description="MCU-friendly MFCC feature extract",
    ...
    def process(self, inputs=None, **kwargs):
        ...
        out_dir = Path('workspace/artifacts') / 'tinyml' / 'mcu_mfcc'
        if stub:
        ...
        try:
            ...
            raise ImportError(f"mcu_mfcc: optional dependency missing ({exc}). Install plugin optional_dependencies or set config.stub=True.") from exc
