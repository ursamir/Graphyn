...
def _config_and_ports(node_type: str):
    """Prefer current PluginPackage AST (repo source of truth).

    Isolated registry stubs are used when they already expose the AST Config
    fields (production UI path). A stale GRAPHYN_HOME install must not hide
    extra_forbidden mismatches in current PluginPackage schemas.
    """
    ...
    if spec is not None:
        ...
        if node_class is not None and getattr(node_class, "_graphyn_isolated", False):
            ast_fields = {name for name, _t, _d in spec.config_fields}
            stub_fields = set(node_class.Config.model_fields)
            if ast_fields and ast_fields <= stub_fields:
                return (
                    node_class.Config,
...
def _all_graph_json_paths() -> list[Path]:
    root = examples_dir()
    return sorted(p for p in root.rglob("*.graph.json") if p.is_file())


@pytest.mark.parametrize(
    "path",
    _all_graph_json_paths(),
    ids=lambda p: str(p.relative_to(examples_dir())).replace("/", "__"),
)
def test_all_example_graph_files_validate_against_registry(path: Path) -> None:
    """Every examples/**/*.graph.json must pass IR + Config + edge compatibility."""
    _validate_graph_like_api(path)
