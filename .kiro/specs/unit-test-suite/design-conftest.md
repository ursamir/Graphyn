...
Full implementation of `unit_test/conftest.py`.
...
- `patch_threads` is `autouse=True` — applies to **every** test in `unit_test/`.
- Tests that need real thread execution (e.g. `test_ingestion.py` thread-safety test) must explicitly override with `@pytest.mark.usefixtures()` or use `patch_threads` as a context manager that they exit before spawning threads.
- `tmp_workspace` is **not** autouse — only tests that touch the filesystem need it.
- `api_client` creates a new `TestClient` per test (function scope) to avoid state leakage between API tests.
